...
class ImplementationTemplateWizard(models.Model):
    ...
    def create_implementation_template(self, target_document_model, document_lines_field_name, template_name, forced_creation=False):
        ...
        for f_key in dmf:
            ...
            some_special_case = False
            if target_document_model == 'purchase.order' and attrs['name'] == 'partner_ref':
                internal_field = "collection_comment"
                some_special_case = True
            if target_document_model == 'sale.order' and attrs['name'] == 'client_order_ref':
                internal_field = "collection_comment"
                some_special_case = True


            # if (is_readonly and not is_product and not is_uom and not is_transfer_picking_type):
            if (is_readonly and not is_product_related and not is_uom and not some_special_case):
                continue
            # if (not is_required and not is_product and not is_uom and not is_name and not is_transfer_picking_type):
            if (not is_required and not is_product_related and not is_uom and not is_name and not some_special_case):
                continue
            if (not is_primitive and not is_product_related and not is_uom and not some_special_case):
                continue
            if is_name:
                internal_field = "name"
            elif is_date:
    ...
    def generate_implementation_template(self):
        ...
        if self.target_document_model == 'product.pricelist':
            ctx['default_forced_creation'] = True
